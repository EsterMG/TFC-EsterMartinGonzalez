...
// SELECTOR DE HORARIO
function crearSelector({ gridId, inpIniId, inpFinId, durId, hiddenIniId, hiddenFinId, def_ini, def_fin }) {
    const SLOTS = 49;
    let sIni = -1, sFin = -1, drag = false;
    ...
    function renderG() {
        grid.querySelectorAll('.celda-h').forEach((c, s) => {
            ...
            if (sIni < 0) return;
            if (sFin < 0) { if (s === sIni) c.classList.add('sel-h'); return; }
    ...
    function renderD() {
        if (!durEl) return;
        if (sIni < 0 || sFin < 0) { durEl.textContent = ''; return; }

        const t = (sFin - sIni) * 30;
        const h = Math.floor(t / 60);
        const m = t % 60;

        durEl.innerHTML = '<strong>' + h + 'h' + (m > 0 ? ' 30min' : '') + '</strong> de turno';
    }
    ...
    return {
        ...
        getIni() { return sIni >= 0 ? s2l(sIni) : def_ini; },
        getFin() { return sFin >= 0 ? s2l(sFin) : def_fin; }
    };
}
...
function aplicarPillCtrl(btn) {
    document.querySelectorAll('.hora-pill-ctrl').forEach(b => b.classList.remove('activo-pill'));
...
function mostrarFormulario() {
    ...
    renderDias();
    document.getElementById('reserva-card').scrollIntoView({ behavior: 'smooth', block: 'start' });
}
...
function limpiarErrores() {
    ocultarErrorInline();
    ['wrap-programa', 'wrap-control', 'wrap-plato', 'wrap-horario-ctrl', 'wrap-fechas', 'wrap-puestos']
...
function validarYEnviar() {
    ...
    let err = null, campo = null;

    if (!programa) { err = 'Debes seleccionar un programa.'; campo = 'wrap-programa'; }
    else if (!control) { err = 'Debes seleccionar un control.'; campo = 'wrap-control'; }
    else if (!plato) { err = 'Debes seleccionar un plato.'; campo = 'wrap-plato'; }
    else if (!ctrlIni || !ctrlFin) { err = 'Indica el horario del control.'; campo = 'wrap-horario-ctrl'; }
    else if (!/^\d{2}:\d{2}$/.test(ctrlIni) || !/^\d{2}:\d{2}$/.test(ctrlFin)) { err = 'Formato de hora inválido (HH:MM).'; campo = 'wrap-horario-ctrl'; }
    else if (!fechas) { err = 'Selecciona al menos un día.'; campo = 'wrap-fechas'; }
    else if (nPuestos === 0) { err = 'Añade al menos un puesto.'; campo = 'wrap-puestos'; }
...
function renderDias() {
    ...
    for (let i = carOffset; i < carOffset + N_DIAS && i < todosLosDias.length; i++) {
        ...
        btn.type = 'button';
        btn.className = 'dia-btn' + (finde && !sel ? ' finde' : '') + (sel ? ' seleccionado' : '');

        btn.innerHTML =
            `<span class="dia-nom">${NOM_DIA[dowES]}</span>
...
function carruselMover(delta) {
    carOffset = Math.max(0, Math.min(carOffset + delta, todosLosDias.length - N_DIAS));
    renderDias();
}
...
function toggleSeleccion(btn) { btn.classList.toggle('activo'); }

function marcarTodos() {
    document.querySelectorAll('#puestos-grid .puesto-item').forEach(b => b.classList.add('activo'));
}

function desmarcarTodos() {
    document.querySelectorAll('#puestos-grid .puesto-item').forEach(b => b.classList.remove('activo'));
}

function añadirSeleccionados() {
    document.querySelectorAll('#puestos-grid .puesto-item.activo')
