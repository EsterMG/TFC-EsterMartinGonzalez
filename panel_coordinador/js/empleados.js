document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelectorAll(".ev-btn-editar").forEach(btn => {
        btn.addEventListener("click", () => {
            ...
            row.querySelectorAll(".ev-vista").forEach(el => el.style.display = "none");
    ...
    btnCancelarNuevo.addEventListener("click", () => modalNuevo.style.display = "none");
    ...
    document.getElementById("emp-borrar-close").addEventListener("click", () => modalBorrar.style.display = "none");
    document.getElementById("emp-borrar-cancel").addEventListener("click", () => modalBorrar.style.display = "none");
    ...
    const camposRequeridos = [
        ...
        { name: "num_empleado", msg: "El número de empleado no puede estar vacío." },
    ...
    camposRequeridos.forEach(({ name }) => {
        const campo = formNuevo ? formNuevo.querySelector(`[name="${name}"]`) : null;
        if (!campo) return;
    ...
    if (formNuevo) {
        formNuevo.addEventListener("submit", e => {
            ...
            camposRequeridos.forEach(({ name, msg }) => {
                ...
                if (!campo) return;
                const vacio = campo.value.trim() === "" || (campo.tagName === "SELECT" && campo.value === "");
                if (vacio) {
                    e.preventDefault();
            ...
            if (emailCampo && emailCampo.value.trim() && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailCampo.value.trim())) {
                e.preventDefault();
            ...
            if (pwdCampo && pwdCampo.value.length > 0 && pwdCampo.value.length < 6) {
                e.preventDefault();
                mostrarError(pwdCampo, "La contraseña debe tener al menos 6 caracteres.");
            }
        });
    }
