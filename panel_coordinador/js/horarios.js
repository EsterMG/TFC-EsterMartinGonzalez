...
function activarTab(ctrl) {
  document
    ...
    .forEach((t) => t.classList.toggle("activa", t.dataset.ctrl === ctrl));
...
function _cerrarConfirmar() {
  cerrarConfirmar();
}

// POPUP CALENDARIO
function abrirCalendario() {
  abrirPopup("ovCalendario");
}
function cerrarCalendario() {
  cerrarPopup("ovCalendario");
}
...
function autoPlato() {
  ...
  if (num && selPlato) {
    for (const o of selPlato.options) {
      if (o.value === "Plato " + num) {
        selPlato.value = o.value;
        break;
      }
    }
  }
...
// Alias usado desde el HTML
function ntAutoPlato() {
  autoPlato();
}
function confirmarCrearTurno() {
  var formNormal = document.querySelector("#nt-panel-normal form");
  var control = formNormal.querySelector('[name="control_nombre"]').value;
  var horaIni = formNormal.querySelector('[name="hora_inicio"]').value;
  var horaFin = formNormal.querySelector('[name="hora_fin"]').value;
  var fechas = document.getElementById("nt-fechas-hidden").value;

  var errores = [];
  if (!control) errores.push("Selecciona un control.");
  if (!horaIni || !horaFin) errores.push("Indica el horario.");
  if (!fechas) errores.push("Selecciona al menos un día.");

  if (errores.length > 0) {
    document.getElementById("ovConfirmar-ico").textContent = "⚠️";
    document.getElementById("ovConfirmar-tit").textContent = "Faltan datos";
    document.getElementById("ovConfirmar-txt").textContent = errores.join(" ");
    var btn = document.getElementById("ovConfirmar-btn");
  ...
  var numDias = fechas.split(",").filter(function (f) {
    return f.trim();
  }).length;
  var prog = formNormal.querySelector('[name="programa_id"]');
  var progNombre =
    prog.options[prog.selectedIndex].value === "0"
      ? "Sin programa"
      : prog.options[prog.selectedIndex].text;

  document.getElementById("ovConfirmar-ico").textContent = "📋";
  document.getElementById("ovConfirmar-tit").textContent =
    "¿Crear estos turnos?";
  document.getElementById("ovConfirmar-txt").textContent =
    "Control: " +
    control +
    " · " +
    horaIni +
  ...
  var btn = document.getElementById("ovConfirmar-btn");
  btn.textContent = "Sí, crear turnos";
  btn.onclick = function () {
    cerrarConfirmar();
    formNormal.submit();
  };
...
function confirmarCrearTurnoAvanzado() {
  var formAv = document.querySelector("#nt-panel-avanzado form");
  var prog = formAv.querySelector('[name="programa_id"]');
  var control = formAv.querySelector('[name="control_nombre"]').value;
  var horaIni = formAv.querySelector('[name="hora_inicio"]').value;
  var horaFin = formAv.querySelector('[name="hora_fin"]').value;
  var fechas = document.getElementById("nt-av-fechas-hidden").value;
  var empleados = document.getElementById("nt-av-empleados-hidden").value;

  var errores = [];
  if (!prog.value) errores.push("Selecciona un programa.");
  if (!control) errores.push("Selecciona un control.");
  if (!horaIni || !horaFin) errores.push("Indica el horario.");
  if (!fechas) errores.push("Selecciona al menos un día.");
  ...
  var numEmps = empleados
    ? empleados.split(",").filter(function (e) {
        return e.trim();
      }).length
    : 0;
  var progNombre = prog.options[prog.selectedIndex].text;

  document.getElementById("ovConfirmar-ico").textContent = "📋";
  document.getElementById("ovConfirmar-tit").textContent =
    "¿Crear y asignar estos turnos?";
  document.getElementById("ovConfirmar-txt").textContent =
    "Programa: " +
    progNombre +
    " · " +
  ...
  var btn = document.getElementById("ovConfirmar-btn");
  btn.textContent = "Sí, crear y asignar";
  btn.onclick = function () {
    cerrarConfirmar();
    formAv.submit();
  };
...
function ntCambiarModo(modo) {
  ...
  document.getElementById("nt-panel-normal").style.display = esAvanzado
    ? "none"
    : "block";
  document.getElementById("nt-panel-avanzado").style.display = esAvanzado
    ? "block"
    : "none";
  document.getElementById("nt-tab-normal").style.borderBottomColor = esAvanzado
    ? "transparent"
    : "var(--texto)";
  document.getElementById("nt-tab-normal").style.color = esAvanzado
    ? "var(--suave)"
    : "var(--texto)";
  document.getElementById("nt-tab-avanzado").style.borderBottomColor =
    esAvanzado ? "var(--texto)" : "transparent";
  document.getElementById("nt-tab-avanzado").style.color = esAvanzado
    ? "var(--texto)"
    : "var(--suave)";

  if (esAvanzado) {
    offsetAvanzado = Math.max(
      0,
      todosDias.findIndex((d) => isoFecha(d) === HOR.fechaSel) - 3,
    );
    ntAvRenderDias();
    const ctrl = getControlActivo();
...
function ntAvDeselTodos() {
  document
    .querySelectorAll(".nt-av-emp-opt.nt-av-sel")
    .forEach((el) => ntAvToggleEmp(el));
}
...
function actualizarEmpsAvanzado() {
  ...
  if (res)
    res.textContent =
      n === 0 ? "0 seleccionados" : `${n} seleccionado${n > 1 ? "s" : ""}`;
}
...
function ntAvCarrusel(delta) {
  offsetAvanzado = Math.max(0, offsetAvanzado + delta);
  ntAvRenderDias();
}
...
function mostrarModoAnadir(ini, fin, puesto) {
  ...
  if (btnBorrar)
    btnBorrar.style.display = ctxAsignar.turnoId > 0 ? "block" : "none";
}
...
function construirListaEmpleados(listaId, puesto) {
  ...
  if (!empleadosFiltrados.length) {
    lista.innerHTML =
      '<div style="font-size:11px;color:var(--suave);padding:8px 0">Sin empleados para este puesto.</div>';
    return;
  }
  ...
  empleadosFiltrados.forEach((e) => {
    ...
    div.addEventListener("click", () => {
      lista
        .querySelectorAll(".wk-opt")
        .forEach((o) => o.classList.remove("wk-sel"));
...
function confirmarAsignacion() {
  ...
  if (ctxAsignar.desdeDrag) {
    ...
    if (bloqueId) {
      ...
      dragPendiente = {
        puesto: ctxAsignar.puesto,
        ctrl: ctxAsignar.ctrl,
        ini,
        fin,
        empId: empleadoSeleccionado,
      };
      abrirContextoDrag();
    }
    return;
  ...
  const campos =
    ctxAsignar.solId > 0
      ? {
          accion: "asignar_sol",
          solicitud_id: ctxAsignar.solId,
          puesto_id: ctxAsignar.puestoSolId,
          empleado_id: empleadoSeleccionado,
          fecha_ctx: HOR.fechaSel,
          mes_ctx: HOR.mesOffset,
        }
      : {
          accion: "asignar_empleado",
...
function guardarCambios() {
  ...
  if (ini && fin) {
    cambiosPendientes.hora_inicio = ini;
    cambiosPendientes.hora_fin = fin;
  }

  document.getElementById("as-edit-trabajador").style.display = "none";
  document.getElementById("as-edit-horario").style.display = "none";

  if (!Object.keys(cambiosPendientes).length) {
    cerrarAsignar();
    return;
  }
...
(function iniciaDragCeldaVacia() {
  ...
  function pxAMin(cel, px) {
    ...
    return (
      Math.round(
        (parseInt(cel.dataset.slotIni) +
          ratio * parseInt(cel.dataset.ventana)) /
          SNAP_MIN,
      ) * SNAP_MIN
    );
  }
  ...
  function pct(cel, m) {
    return Math.max(
      ...
      Math.min(
        100,
        ((m - parseInt(cel.dataset.slotIni)) / parseInt(cel.dataset.ventana)) *
          100,
      ),
    );
  }
  ...
  function mostrarPreview(cel, m0, m1) {
    if (!preview) {
      ...
      preview.className = "g-drag-preview";
    }
    if (preview.parentElement !== cel) {
      preview.remove();
      cel.appendChild(preview);
    }
    preview.style.left = pct(cel, Math.min(m0, m1)) + "%";
    preview.style.width =
      pct(cel, Math.max(m0, m1)) - pct(cel, Math.min(m0, m1)) + "%";
  }
  ...
  document.addEventListener("mouseup", (e) => {
    if (!arrastrando || !celda) return;
    arrastrando = false;
    const m0 = pxAMin(celda, xInicio),
      m1 = pxAMin(celda, e.clientX);
    ocultarPreview();
    if (Math.abs(m1 - m0) < SNAP_MIN) {
      celda = null;
      return;
    }

    const ini = minHH(Math.min(m0, m1)),
      fin = minHH(Math.max(m0, m1));
    const puesto = celda.dataset.puesto,
      ctrl = celda.dataset.ctrl;
    celda = null;
...
(function iniciaResizeBarra() {
  ...
  document.addEventListener("mousemove", (e) => {
    ...
    if (txt)
      txt.textContent = minHH(newIni % 1440) + "–" + minHH(newFin % 1440);
  });
